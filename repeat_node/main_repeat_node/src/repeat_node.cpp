...
// Main variables	
	...
	geometry_msgs::PoseStamped gmOLhat;
...
// INS variables
	...
	Mat SCRChat_ins = Mat::zeros(3,1,CV_64F);
...
// ICP variables
	...
	Mat SCRChat_icp = Mat::zeros(3,1,CV_64F);
	Mat QCRhat_icp = Mat::zeros(3,1,CV_64F);
...
// Localisation node variables
	Mat SReRRhat = Mat::zeros(3,1,CV_64F);
	Mat QReRhat = Mat::zeros(4,1,CV_64F);
	Mat errTOR = Mat::zeros(3,3,CV_64F);	
	
typedef actionlib::SimpleActionClient<hector_uav_msgs::PoseAction> PoseActionClient;

// Function prototypes
void updateState(Mat inSBLLhat, Mat inQBLhat, Mat inSRLLhat, Mat inQRLhat, Mat inSCRChat, double weight);
void resetMap();
void setVO();
void waypointNav();
...
// -------------------------------------------------------------
/*
localisationErrorCallBack - Obtains and processes error from localisation node

Author: JDev 161220
	
*/
// -------------------------------------------------------------
void localisationErrorCallBack(const geometry_msgs::PoseStamped::ConstPtr& gmReRhat){
	// Local variables
	stringstream currentMapName;
	
	// Process
	currentMapName << "map_" << mapCounter;
	
	// If header matches current map name, and if system isn't moving to the next submap
	if ((gmReRhat->header.frame_id.compare(currentMapName.str()) == 0) && !next_map){
		SReRRhat.at<double>(0) = gmReRhat->pose.position.x;
		SReRRhat.at<double>(1) = gmReRhat->pose.position.y;
		SReRRhat.at<double>(2) = gmReRhat->pose.position.z;
		QReRhat.at<double>(0) = gmReRhat->pose.orientation.x;
		QReRhat.at<double>(1) = gmReRhat->pose.orientation.y;
		QReRhat.at<double>(2) = gmReRhat->pose.orientation.z;
		QReRhat.at<double>(3) = gmReRhat->pose.orientation.w;
		ROS_INFO("QReRhat: [%f,%f,%f,%f]", QReRhat.at<double>(0), QReRhat.at<double>(1), QReRhat.at<double>(2), QReRhat.at<double>(3));
		ROS_INFO("SReRRhat: [%f,%f,%f]", SReRRhat.at<double>(0), SReRRhat.at<double>(1), SReRRhat.at<double>(2));	
	} else {
		// Set error to zero
		//SReRRhat = Mat::zeros(3,1,CV_64F);
		//QReRhat = Mat::zeros(4,1,CV_64F);
		//QReRhat.at<double>(3) = 1.0;
	}
...
*/
// -------------------------------------------------------------
void localisationUpdateCallBack(const geometry_msgs::PoseStamped::ConstPtr& gmRLhat){
	// Local variables
	stringstream currentMapName;
	
	// Read in gmerrOR to errSORO and errTOR;
	// TODO: RENAME errSROO to SReRR and errQRO to QReR.t()
	SRLLhat_icp.at<double>(0) = gmRLhat->pose.position.x;
	SRLLhat_icp.at<double>(1) = gmRLhat->pose.position.y;
	SRLLhat_icp.at<double>(2) = gmRLhat->pose.position.z;
	QRLhat_icp.at<double>(0) = gmRLhat->pose.orientation.x;
	QRLhat_icp.at<double>(1) = gmRLhat->pose.orientation.y;
	QRLhat_icp.at<double>(2) = gmRLhat->pose.orientation.z;
	QRLhat_icp.at<double>(3) = gmRLhat->pose.orientation.w;
	currentMapName << "map_" << mapCounter;
	
	// Normalise errQOR
	//errQRO = quatnormalise(errQRO);
	cout << "currentMapName: " << currentMapName.str() << endl;
	cout << "gmRLhat->header: " << gmRLhat->header.frame_id << endl;
	// Process only if gmRLhat is from the same map as the current map
	if (gmRLhat->header.frame_id.compare(currentMapName.str()) == 0){
		validICP = true;
		...
		SCRChat_icp = TCLhat_icp*(SCLLhat_icp - SRLLhat_icp);
		QCRhat_icp = dcm2quat(quat2dcm(TCLhat_icp)*quat2dcm(QRLhat_icp).t());
		
		//SCRChat_icp = TCLhat_icp*(TCLhat_icp.t()*SCRChat + quat2dcm(QRLhat_icp).t()*SReRRhat);
		//QCRhat_icp = dcm2quat(quat2dcm(QCRhat)*quat2dcm(QReRhat));
		
		
		ROS_INFO("SCRChat_icp: [%f,%f,%f]", SCRChat.at<double>(0), SCRChat.at<double>(1), SCRChat.at<double>(2));
		...
		//SBLLhat = SBLLhat_icp.clone();
		//QBLhat = QBLhat_icp.clone();
		//SRLLhat = SRLLhat_icp.clone();
		//QRLhat = QRLhat_icp.clone();
		if (next_map){
			updateState(SBLLhat, QBLhat, SRLLhat_icp, QRLhat_icp, SCRChat, 0.2);
		} else if ((fabs(norm(SReRRhat)) > 0) && (fabs(norm(SReRRhat)) < 0.01)) {
			updateState(SBLLhat, QBLhat, SRLLhat_icp, QRLhat_icp, SCRChat, 1.0);
		} else {
			updateState(SBLLhat, QBLhat, SRLLhat_icp, QRLhat_icp, SCRChat, 0.5);
		}
		//resetMap();
	} else {
		//validICP = false;
...
void voCallBack(const nav_msgs::Odometry::ConstPtr& odomMsg){
	...
	if ( fabs(norm(SCRChat_vo)) == 0){
		...
		// IF the map is not currently being reset (i.e. SCRC/QCR not being set to zero.)
		if (!next_map){
			// Apply error terms to QCRhat_vo
			if (validICP) {
				//QCRhat_vo = dcm2quat(quat2dcm(QCRhat_vo)*quat2dcm(QReRhat));	
				// Apply error terms to SCRChat
				//SCRChat_vo = SCRChat_vo + quat2dcm(QCRhat_vo)*SReRRhat;

			}
			ROS_INFO("QReRhat_vo: [%f,%f,%f,%f]", QReRhat.at<double>(0), QReRhat.at<double>(1), QReRhat.at<double>(2), QReRhat.at<double>(3));
			ROS_INFO("SReRRhat_vo: [%f,%f,%f]", SReRRhat.at<double>(0), SReRRhat.at<double>(1), SReRRhat.at<double>(2));	
		
			// Use VO alone to update state
			TCLhat_vo = quat2dcm(QCRhat_vo)*quat2dcm(QRLhat);	// TCB*quat2dcm(QBLhat);					// QBLhat <- state update
			QRLhat_vo = dcm2quat(quat2dcm(QCRhat_vo).t()*TCLhat_vo);
			...
			if (validICP) {
				updateState(SBLLhat_vo, QBLhat_vo, SRLLhat_vo, QRLhat_vo, SCRChat_vo, 0.5);	// dcm2quat(TRLhat_vo)
			} else {
				updateState(SBLLhat_vo, QBLhat_vo, SRLLhat_vo, QRLhat_vo, SCRChat_vo, 0.5);	// dcm2quat(TRLhat_vo)
			}
		}
...
void insCallBack( const geometry_msgs::PoseStamped::ConstPtr& extPose ){
	...
	SCRChat_ins = quat2dcm(QCLhat_ins)*SCRLhat_ins;
	// TRL = TRC*TCB*TBL
	...
	// Send to state update
	if (validICP) {
		updateState(SBLLhat_ins, QBLhat_ins, SRLLhat_ins, QRLhat_ins, SCRChat_ins, 0.3);
	} else {
		updateState(SBLLhat_ins, QBLhat_ins, SRLLhat_ins, QRLhat_ins, SCRChat_ins, 0.3);	
	}
}
...
void generateWaypoints(string poseFileName){
	...
	if(poseFile.fail()){
		...
		for (int i = 0; i < vecSCOL.size(); i++){
			...
			QCL_cmd = quatnormalise(vecQCL[i]);
			// Geometry message
...
void loadSubmapPCD(string PCDFileName){
	...
	// Set header for submapCloudPC2
	mapCounterSS << mapCounter;
	submapCloudPC2.header.frame_id = "map_" + mapCounterSS.str();
}
...
bool initVars(){
	...
	SCRChat = quat2dcm(QCLhat)*(SCLLhat - SRLLhat);
	...
	if (fabs(norm(SBLL)) > 0){
		...
		updateState(SBLLhat, QBLhat, SRLLhat_vo, QRLhat_vo, SCRChat, 0.5);	
...
void updateState(Mat inSBLLhat, Mat inQBLhat, Mat inSRLLhat, Mat inQRLhat, Mat inSCRChat, double weight){
	...
	SBLLhat = (1.0-weight)*SBLLhat + weight*inSBLLhat;
	QBLhat = (1.0-weight)*QBLhat + weight*inQBLhat;
	SRLLhat = (1.0-weight)*SRLLhat + weight*inSRLLhat;
	QRLhat = (1.0-weight)*QRLhat + weight*inQRLhat;
	SCRChat = (1.0-weight)*SCRChat + weight*inSCRChat;
	...
	//ROS_INFO("SOLLhat: [%f,%f,%f]", SOLLhat.at<double>(0), SOLLhat.at<double>(1), SOLLhat.at<double>(2));
	...
	//ROS_INFO("SRORhat: [%f,%f,%f]", SRORhat.at<double>(0), SRORhat.at<double>(1), SRORhat.at<double>(2));
	//ROS_INFO("QROhat: [%f,%f,%f,%f]", QROhat.at<double>(0), QROhat.at<double>(1), QROhat.at<double>(2), QROhat.at<double>(3));
	//ROS_INFO("SCRChat: [%f,%f,%f]", SCRChat.at<double>(0), SCRChat.at<double>(1), SCRChat.at<double>(2));
	...
	// Set up SOLLhat/QOLhat for broadcast
	gmOLhat.pose.position.x = SOLLhat.at<double>(0);
	gmOLhat.pose.position.y = SOLLhat.at<double>(1);
	gmOLhat.pose.position.z = SOLLhat.at<double>(2);
	gmOLhat.pose.orientation.x = QOLhat.at<double>(0);
	gmOLhat.pose.orientation.y = QOLhat.at<double>(1);
	gmOLhat.pose.orientation.z = QOLhat.at<double>(2);
	gmOLhat.pose.orientation.w = QOLhat.at<double>(3);
	// gmOLhat header
	gmOLhat.header.stamp = ros::Time::now();
	gmOLhat.header.seq++;
	mapCounterSS << mapCounter;
	gmOLhat.header.frame_id = "map_" + mapCounterSS.str();	
	
	// Reset VO to reflect update to SCRChat
	setVO();
	
	// Set broadcastState to true
	broadcastState = true;
...
void nextSubmap(){
	...
	// TODO DEBUG: This resets back to 0 when first submap complete.
	//if (mapCounter < 1){
		mapCounter++;
	//} else {
	//	mapCounter = 0;
	...
	ROS_INFO_STREAM("Current map name: " << currentMapName.str());
	...
	// Set SReRR to ones
	SReRRhat = Mat::ones(3,1,CV_64F);*/
	
		// Initialise state variables
	// SBLLhat, QBLhat
	SBLLhat = SBLLhat.clone(); QBLhat = QBLhat.clone();
	SCLLhat = quat2dcm(QBLhat).t()*SCBB+SBLLhat;
	QCLhat = dcm2quat(TCB*quat2dcm(QBLhat));
	SRLLhat = quat2dcm(QBLhat).t()*SCBB+SBLLhat;
	QRLhat = dcm2quat(TCB*quat2dcm(QBLhat));
	SCRChat = quat2dcm(QCLhat)*(SCLLhat - SRLLhat);
	
	// Initialise INS variables
	// SCLLhat_ins, QCLhat_ins, SBLLhat_ins, QBLhat_ins
	SBLLhat_ins = SBLLhat.clone();
	QBLhat_ins = QBLhat.clone();
	SCLLhat_ins = quat2dcm(QBLhat).t()*SCBB+SBLLhat;
	QCLhat_ins = dcm2quat(TCB*quat2dcm(QBLhat));
	SCRChat_ins = SCRChat.clone();
	
	// Initialise VO variables
	// SCLLhat_vo, QCLhat_vo, SBLLhat_vo, QBLhat_vo, SRLLhat_vo, QRLhat_vo
	SBLLhat_vo = SBLLhat.clone();
	QBLhat_vo = QBLhat.clone();
	SCLLhat_vo = quat2dcm(QBLhat).t()*SCBB+SBLLhat;
	QCLhat_vo = dcm2quat(TCB*quat2dcm(QBLhat));
	SRLLhat_vo = quat2dcm(QBLhat).t()*SCBB+SBLLhat;	// SRLL = initial SCLL
	QRLhat_vo = dcm2quat(TCB*quat2dcm(QBLhat));	// QRL = initial QCL
	SCRChat_vo = SCRChat.clone();
	
	ROS_INFO_STREAM("TBL init: " << quat2dcm(QBLhat));
	ROS_INFO_STREAM("TRLhat_vo init: " << quat2dcm(QRLhat_vo));
	ROS_INFO_STREAM("TCB init: " << TCB);
	
	// Initialise ICP variables
	SBLLhat_icp = SBLLhat.clone();
	QBLhat_icp = QBLhat.clone();
	SCLLhat_icp = quat2dcm(QBLhat).t()*SCBB+SBLLhat;
	QCLhat_icp = dcm2quat(TCB*quat2dcm(QBLhat));
	SRLLhat_icp = quat2dcm(QBLhat).t()*SCBB+SBLLhat;
	QRLhat_icp = dcm2quat(TCB*quat2dcm(QBLhat));	
	SCRChat_icp = SCRChat.clone();
	...
	ROS_INFO("SRLLhat (reset): [%f,%f,%f]", SRLLhat.at<double>(0), SRLLhat.at<double>(1), SRLLhat.at<double>(2));
	ROS_INFO("SOLLhat (reset): [%f,%f,%f]", SOLLhat.at<double>(0), SOLLhat.at<double>(1), SOLLhat.at<double>(2));	
	...
	// update state
	for (int i = 0; i < 20; i++){
		updateState(SBLLhat, QBLhat, SRLLhat, QRLhat, SCRChat, 0.5);	
	}
	...
	// Set system to assume ICP output is not valid till previous alignment is complete.
	doICP = true;
	validICP = false;
	resetMap();
...
void waypointNav(){
	...
	//ROS_INFO_STREAM("SReRRhat: " << SReRRhat); 
	...
	if( fabs( SBLLhat.at<double>(0) -  SBLL_cmd.at<double>(0) ) < wp_radius && fabs( QBLhat.at<double>(0) -  QBL_cmd.at<double>(0) ) < 0.15) {
		if( fabs( SBLLhat.at<double>(1) - SBLL_cmd.at<double>(1))  < wp_radius && fabs( QBLhat.at<double>(1) -  QBL_cmd.at<double>(1) ) < 0.15) {
			if( fabs( SBLLhat.at<double>(2) - SBLL_cmd.at<double>(2) )  < wp_radius && fabs( QBLhat.at<double>(2) -  QBL_cmd.at<double>(2) ) < 0.15) {
				if (validICP && (fabs(norm(SReRRhat)) > 0) && (fabs(norm(SReRRhat)) < 0.01) 
					&& (fabs(QReRhat.at<double>(3)) - 1.0 < 0.0005) ){ 	//fabs(norm(errSROO)) < 0.1
...
// -------------------------------------------------------------
/*
setVO -	Resets visual odometry to pose set by state
...
int main(int argc, char **argv){
	...
	submapPosePub = nh.advertise<geometry_msgs::PoseStamped>("repeat_node/submapPose", 1);
	
	// Publish to repeat_node/submapPose
	...
	// Post-init subscribe to localisation error
	localisationErrorSub = nh.subscribe("repeat_node/locErr", 1, localisationErrorCallBack);
	
	// MAIN LOOP
	while (!exitRepeat && nh.ok()){
		ros::spinOnce();
		if (next_map || clearMap){
			next_map = false;
			if(!resetOdometryToZero.call(emptySrv)){
				ROS_INFO("[repeat_node] Failed to reset odometry to zero.");
			} else {
				ROS_INFO("[repeat_node] Succeeded in resetting odometry to zero..");				
			}
		...
		// If VO has been lost for 5 frames or more, reset VO, or if new map is requested
		if (setVOToPose){	//  || (VOLossCounter > 5)
			setVOToPose = false;
			// Send reset request
			if (!resetOdometryClient.call(odomResetReq, odomResetResp)){
				ROS_INFO("[repeat_node] Failed to reset odometry to pose.");
			} else {
				ROS_INFO("[repeat_node] Succeeded in resetting odometry to pose.");
				// Reset loss counter
				VOLossCounter = 0;
		...
		// Broadcast submap pose
		submapPosePub.publish(gmOLhat);
