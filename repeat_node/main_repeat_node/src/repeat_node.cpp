...
void voCallBack(const nav_msgs::Odometry::ConstPtr& odomMsg){
	...
	if ( fabs(norm(SCRChat_vo)) == 0){
		...
		SCLLhat_vo = TCLhat_vo.t()*SCRChat_vo + SRLLhat_vo; //TCLhat_vo.t()*TCB*SCBB + SBLLhat;		// SRLLhat <- state update
		...
		updateState(SBLLhat_vo, QBLhat_vo, SRLLhat_icp, QRLhat_icp);	// dcm2quat(TRLhat_vo)
	}
...
void insCallBack( const geometry_msgs::PoseStamped::ConstPtr& extPose ){
	...
	// Send to state update
	updateState(SBLLhat_ins, QBLhat_ins, SRLLhat_icp, QRLhat_icp);
}
