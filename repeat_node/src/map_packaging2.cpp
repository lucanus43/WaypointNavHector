...
*/
// -------------------------------------------------------------
void packageMap(vector<Mat> vecSCOL, vector<Mat> vecTCL, Mat SOLL, string poseFileName){	
	// Local variables	
...
bool extractPosesFromFile(fstream &poseFile, Mat &SOLL, vector<Mat> &vecSCOL, vector<Mat> &vecTCL){
	// Local variables
	string tempString;
	size_t pos = 0;
	string token;
	...
	if (!poseFile.eof()) {
		...
		// Push back vectors
		vecSCOL.push_back(SCOL);
		vecTCL.push_back(quat2dcm(QCL));
		return true;
	} else { return false; }
}
