...
void odomCallBack(const nav_msgs::Odometry::ConstPtr& odomMsg){
	...
	cout << "SOCLhat: " << SOCLhat << endl;
	vecSOCL.push_back(SOCLhat.clone());
	vecTCL.push_back(TCLhat.clone());
	
	// TODO: why is vecSOCL getting the same SOCLhat value pushed to all of it?
	// debug - output vecSOCL to screen
		cout << "vecSOCL[" << vecSOCL.size()-1 << "]: " << vecSOCL[vecSOCL.size()-1] << endl;
}
...
int main(int argc, char **argv){
	...
	while(nh.ok()){
		...
		if (fabs(norm(SOCLhat)) > 2.0){		// |SOCL| > 2 m
			...
			if (resetVars){
			// debug 
				for (int j = 0; j < vecSOCL.size(); j++){
					cout << "vecSOCL[" << j << "]: " << vecSOCL[j] << endl;
				}
