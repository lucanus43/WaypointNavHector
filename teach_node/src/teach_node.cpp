...
void cloudCallBack(const boost::shared_ptr<const sensor_msgs::PointCloud2>& input) {
    ...
    if (saveMap){
		...
		packageMap(vecSCOL, vecTCL, SOLLhat, QOLhat, posefileName.str());
...
void odomCallBack(const nav_msgs::Odometry::ConstPtr& odomMsg){
	...
	if ( fabs(norm(SCOChat_vo)) == 0){
		...
		ROS_INFO_STREAM("[teach_node] SCOLhat: " << SCOLhat_vo);
		ROS_INFO_STREAM("[teach_node] SOLLhat: " << SOLLhat_vo);
		// Output SCOL to WS
...
void updateState(Mat inSBLLhat, Mat inQBLhat, Mat inSOLLhat, Mat inQOLhat){
	...
	vecTCL.push_back(quat2dcm(QCLhat).clone());
}
